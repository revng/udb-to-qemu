...
struct BitRange {
    ...
    BitRange(T v) : value(static_cast<uint64_t>(v)), length(std::max(1, std::bit_width(static_cast<std::make_unsigned_t<T>>(v)))) {}

    template<uint32_t N, bool S>
    BitRange(Bits<N,S> &b) : value(b.value()), length(N) {}
};
...
template<uint32_t N, bool S = false>
class __attribute__((packed)) Bits {
public:
...
public:
    ...
    Bits<1> operator[](size_t i) {
        return (larger_value >> i) & 1;
    }

    template<uint32_t M, bool T>
    Bits<1> operator[](Bits<M,T> reg) {
        return (larger_value >> reg.value()) & 1;
    }

    template<uint32_t b, uint32_t e>
    Bits<e-b+1> range() {
