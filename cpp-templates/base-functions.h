...
constexpr std::make_signed_t<T> signed_ext(T value, int start, int length) {
...
void delay(Bits<N,S> imm) {
}

template<uint32_t N, bool S>
__attribute__((always_inline))
constexpr Bits<N,true> _signed(Bits<N, S> b) {
    using T = typename Bits<N,S>::type;
    if constexpr (N < 8*sizeof(T)) {
        return {static_cast<typename Bits<N,true>::type>(signed_ext<T>(b.value(), 0, N))};
    } else {
...
template<typename T, typename = std::enable_if_t<std::is_integral_v<T>>>
__attribute__((always_inline))
constexpr std::make_signed_t<T> _signed(T t) {
    return t;
}

template<uint32_t count, uint32_t N, bool S>
__attribute__((always_inline))
constexpr Bits<count*N, S> repeat(const Bits<N,S> b) {
...
Csr direct_csr_lookup(Bits<32> no) {
    return no;
}
...
void ann_xqci_jump_pcrel(int imm) {}
__attribute__((annotate ("immediate: 0")))
void xqci_jump_pcrel(int imm);
__attribute__((annotate ("returns-immediate")))
uint64_t ann_xqci_current_pc() {return 0;}
__attribute__((annotate ("returns-immediate")))
uint64_t xqci_current_pc();
...
__attribute__((annotate ("immediate: 1")))
int32_t ann_xqci_csrr(CPUArchState *, int32_t csrno) {return 0;}
__attribute__((annotate ("immediate: 1")))
...
template<uint32_t N, bool S>
XReg maybe_sext_xreg(Bits<N,S> b) {
    if constexpr (N < 32) {
        using T = typename Bits<N,S>::type;
...
template<uint32_t N, bool S>
void xqci_jump_pcrel_bits(Bits<N,S> imm) {
...
int32_t csr_sw_read(CPUArchState *env, Csr no) {
    return xqci_csrr_xreg(env, no);
}
...
template<uint32_t N, bool S, typename T, typename = std::enable_if_t<std::is_integral_v<T>>>
...
template<int64_t amount, uint32_t N, bool S,
         typename ResT = Bits<N + amount, S>>
